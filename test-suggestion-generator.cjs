...
try {
  await generator.generateSuggestions('', createMockTimeRange());
  console.log('✓ Empty transcript returns empty array');
} catch (error) {
  console.error('✗ Empty transcript test failed:', error.message);
}

try {
  await generator.generateSuggestions('test', null);
...
try {
  const suggestions = await generator.generateSuggestions(
    'We need to continue working on the user system',
    createMockTimeRange(),
    previousSuggestions,
    conversationContext
  );
...
const duplicateTranscript = 'We need to implement authentication system';
...
try {
  ...
  const secondBatch = await generator.generateSuggestions(
    'We should implement the authentication system',
    createMockTimeRange(),
    firstBatch
  );
...
try {
  ...
  await statsGenerator.generateSuggestions(
    'We need to implement features and ask questions about the system',
    createMockTimeRange()
  );
...
try {
  ...
  await persistenceGenerator.generateSuggestions(
    'Test transcript for persistence',
    createMockTimeRange()
  );
...
console.log('\n=== All Tests Completed ===');
console.log('Run this file with: node test-suggestion-generator.js');