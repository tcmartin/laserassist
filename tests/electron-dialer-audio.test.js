...
test('actual index dialer sends generated buyer audio through media, ASR and coaching hooks', {
  ...
  try {
    ...
    const rtp = await waitUntil(() => renderer.evaluate(`(async()=>{const peer=window.__audioMetrics.other;let received=0,sent=0;for(const stat of (await peer.getStats()).values()){if(stat.type==='inbound-rtp'&&stat.kind==='audio')received+=stat.bytesReceived||0;if(stat.type==='outbound-rtp'&&stat.kind==='audio')sent+=stat.bytesSent||0;}return {connected:peer.connectionState==='connected',received,sent};})()`, true), (value) => value.connected && value.received > 500 && value.sent > 500, 15000, 'bidirectional real RTP');
    ...
    const evidence = { calls: 1, fake_adapter: { starts: state.starts, ends: state.ends, closes: state.closes }, audio: { answer_sent: metrics.answerSent, remote_track: metrics.remoteTrack, rtp, asr_chunks: proxy.metrics.asrChunks, max_abs_sample: Number(proxy.metrics.asrMaxAbs.toFixed(5)), cleaned: finalMetrics.cleaned }, asr: { fake_stream_starts: proxy.metrics.asrStarts, fake_chunks: proxy.metrics.asrChunks, fake_stream_ends: proxy.metrics.asrEnds }, coaching: { analyses: proxy.metrics.analyses, live_panel: true, rendered_summary: true }, provider_calls: 0, janus_calls: 0, sip_calls: 0 };
